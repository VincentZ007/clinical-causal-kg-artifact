...
\item A controlled causal-RAG study in which lift-only retrieval is the honest base after the temporal-direction audit. A conservative causal-demotion policy improves overall accuracy and sharply reduces unsupported hallucinated answers without lowering retrieval recall.
\end{enumerate}
...
5~Raw-causal & 34.3 & 17.4 & 25.9 & 82.6 & 17.4 & 72.3 \\
...
The broad comparison shows that graph retrieval is substantially better than text-only retrieval or closed-book generation. The support-aware comparison is the cleaner ablation after the audit: lift-only retrieval avoids the refuted temporal flip, and causal demotion preserves the same retrieval recall while improving every correctness axis. The overall gain over lift-only is modest (+1.5 points; McNemar $p=0.27$), so we do not claim a statistically significant accuracy improvement. The robust effect is faithfulness: unsupported hallucinated answers fall from 17.3\% to 6.1\% at the same 86.3\% retrieval recall.

\begin{figure}[!htbp]\centering
\includegraphics[width=0.62\textwidth]{fig_qa.pdf}
\caption{Causal-QA correctness on held-out patients. The final system uses lift-only retrieval and demotes candidates that fail the causal screen, improving overall accuracy over the honest lift-only base while sharply reducing unsupported hallucinated answers.}
\label{fig:qa}
\end{figure}
