...
\smallskip\noindent\textbf{Causal-screen demotion.} The demotion layer is a post-retrieval ordering rule, not a new retriever. For each question, lift-only retrieval first selects the same top-12 candidates by co-occurrence lift. The causal screen is then queried only for these directed pairs: for WHY, each candidate cause is paired with the queried effect; for WHATCAUSES, the queried cause is paired with each candidate effect. The screen is estimated on the training split as a new-user, incident-outcome design over structured diagnoses: a cause must occur at baseline, and an effect counts only if it first appears later among outcome-naive patients. The adjusted risk-ratio signal uses inverse-propensity weighting for age, sex, and baseline comorbidity burden, then calibrates against negative-control outcomes to produce a calibrated $z$ score. A candidate \emph{passes} if its calibrated $z$ is at least \texttt{Z\_THRESH}, \emph{fails} if it has an estimate below this pre-specified cutoff, and is \emph{untested} if sample-size or coverage requirements prevent estimation. We use 1.64 as a one-sided 5\% screening threshold. The cutoff, screen, and QA prompts are fixed before answer generation and are not tuned on held-out QA answers. In demotion mode, pass and untested candidates keep their original lift order, while failed candidates are moved to the end of the same top-12 list. Thus demotion can change which evidence the generator sees first, but it cannot add or remove a candidate, so retrieval recall is preserved by construction.
...
\noindent\textbf{Other limitations.} We use a single 7B open model and English ICU notes. The downstream QA benchmark is built on MIMIC-IV; MIMIC-III is used to test whether the support and direction signals replicate across an older EHR release, not to duplicate the full QA benchmark. This keeps the external experiment aligned with the central audit question. Absolute QA numbers are modest: the clearest downstream effect is reduced unsupported hallucination, while exact concept accuracy shows a non-significant upward trend. The text-RAG row is not intended to represent the strongest possible clinical text retriever. We include two stricter text-only sanity checks in the same harness, but we do not claim to exhaust strong clinical text retrieval baselines such as BM25, domain-specific dense retrieval, or hybrid retrieval. The direct comparison with raw-causal retrieval is therefore the cleaner ablation for our main claim. Lift covers diagnosis-coded endpoints, so causes involving drugs, adverse events, and clinical findings are presently out of scope; integrating prescription and adverse-event signals, as well as multi-hop causal chains, would extend coverage. SemMedDB corroborates only a minority of candidates, limiting literature-based direction to that subset. Co-occurrence lift is an association signal; it supports an extracted candidate but cannot by itself establish a true causal relation.
