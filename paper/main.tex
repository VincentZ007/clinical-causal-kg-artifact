...
\item A controlled causal-RAG study in which lift-only retrieval is the honest base after the temporal-direction audit. A conservative causal-demotion policy preserves recall, sharply reduces unsupported hallucinated answers, and gives a small measured accuracy gain.
\end{enumerate}
...
\caption{Overview. \textbf{Build:} a graph of causal candidates is mined from clinical notes (Qwen + UMLS). \textbf{Audit:} each candidate relation is scored against independent literature evidence (SemMedDB): co-occurrence lift filters unsupported candidates, cross-admission coding-order temporal precedence is unreliable as a direction signal (54\% vs.\ 74\%), and literature corroboration is partial. \textbf{Apply:} support-aware causal RAG uses the EHR-supported candidate graph and demotes candidates that fail the causal screen to answer held-out causal questions.}
\label{fig:overview}
\end{figure}
...
\caption{Causal-QA correctness on held-out patients. The final system uses lift-only retrieval and demotes candidates that fail the causal screen, giving a small measured accuracy gain over the honest lift-only base while sharply reducing unsupported hallucinated answers.}
\label{fig:qa}
\end{figure}
