...
\smallskip\noindent\textbf{Causal-screen demotion.} The demotion layer is applied only after the same top-12 lift-ranked candidates have been selected. For a directed pair (cause CUI, effect CUI), the screen uses structured records to form a new-user cohort when possible: the cause is observed at a baseline admission, the effect is tested as an incident later-admission outcome among outcome-naive patients, and the adjusted risk-ratio signal is calibrated against negative-control outcomes. A candidate \emph{passes} if its calibrated $z\geq1.64$, \emph{fails} if it has a screen estimate below this pre-specified cutoff, and is \emph{untested} if the screen cannot estimate it. The cutoff and screen are fixed before QA generation and are not tuned on held-out QA answers. Demotion then moves only failed candidates behind pass and untested candidates. It does not change the top-12 candidate set, so retrieval recall is preserved by construction.
...
\caption{Causal-QA correctness on held-out patients. The final system uses lift-only retrieval and demotes candidates that fail the causal screen. The main downstream effect is fewer unsupported hallucinated answers at fixed recall; the exact-accuracy change over lift-only is a small non-significant trend.}
\label{fig:qa}
\end{figure}
